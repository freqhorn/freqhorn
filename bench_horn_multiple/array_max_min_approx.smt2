...
(rule (=> (and (inv2 a b i min max N) (< i N)
    (= (store b i
        (ite (< (- max (select a i)) (- (select a i) min)) max min)) b1)
    (= i1 (+ i 1))) (inv2 a b1 i1 min max N)))

(rule (=> (and (inv2 a b i min max N) (>= i N) (< 0 i1) (< i1 N)
    (not (or (= (select b i1) min) (= (select b i1) max)))) fail))
